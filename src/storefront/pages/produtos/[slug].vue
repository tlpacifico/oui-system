...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="text-sm text-gray-500 mb-6 font-sans">
      <NuxtLink to="/produtos" class="hover:text-sage-dark">Produtos</NuxtLink>
    ...
    <div v-if="error" class="text-center py-20">
      ...
      <NuxtLink to="/produtos" class="text-sage-dark hover:underline">Voltar aos produtos</NuxtLink>
    </div>

    <div v-else-if="product" class="grid grid-cols-1 md:grid-cols-2 gap-10">
      <!-- Photos -->
      <div>
        <div class="aspect-square bg-cream-dark rounded-lg overflow-hidden mb-3">
          <img
        ...
        <div v-if="product.photos?.length > 1" class="flex gap-2 overflow-x-auto">
          <button
            ...
            :class="idx === currentPhoto ? 'border-sage' : 'border-cream-dark'"
      ...
      <div>
        ...
        <h1 class="text-2xl font-normal text-gray-900 mb-2">{{ product.title }}</h1>
        <p class="text-3xl font-bold text-sage mb-6">&euro;{{ product.price.toFixed(2) }}</p>
        ...
        <div v-if="product.description" class="mb-8">
          <h2 class="text-sm font-bold text-gray-900 mb-2 font-sans">Descrição</h2>
        ...
        <div>
          <button
            ...
            class="w-full bg-sage text-white py-3 rounded-lg font-semibold hover:bg-sage-dark transition text-base"
            @click="addToCart"
          >
            Adicionar ao Carrinho
          </button>
          <div v-else class="space-y-3">
            <div class="w-full bg-sage/10 text-sage py-3 rounded-lg font-semibold text-center text-base">
              Adicionado ao carrinho
            </div>
            <NuxtLink
              to="/carrinho"
              class="block w-full border-2 border-sage text-sage py-3 rounded-lg font-semibold text-center hover:bg-sage/10 transition text-base"
            >
              Ver Carrinho
            </NuxtLink>
          </div>
